...
class cartOrderService{
    ...
    async order(user_id,customer_name,phone,email,delivery_address,remark,payment_method,tax,delivery_fee,items,file){
        ...
        if(!tax)throw new AppError('Tax is required',400);
        if(!delivery_fee)throw new AppError('Delivery Fee is required',400);
        if(!items)throw new AppError('Items is required',400);
        if(!file)throw new AppError('File is required',400);

        const result = await repo.order(user_id,customer_name,phone,email,delivery_address,remark,payment_method,tax,delivery_fee,items,file); 
